...
class RiskForm(forms.ModelForm):
    ...
    description = forms.CharField(label='Описание риска',
...
class IMNSForm(forms.ModelForm):
    ...
    number = forms.IntegerField(label='Номер инспекции',
                                widget=forms.TextInput(attrs={
                                    'placeholder': 'Номер инспекции',
                                    'class': 'form-control'
                                }))
    name = forms.CharField(label='Полное название инспекции',
                           widget=forms.TextInput(attrs={
                                'placeholder': 'Полное название инспекции',
                                'class': 'form-control'
                           }))
    shot_name = forms.CharField(label='Короткое название инспекции',
                                widget=forms.TextInput(attrs={
                                  'placeholder': 'Короткое название инспекции',
                                  'class': 'form-control'
                                }))
    address = forms.CharField(label='Адресс инспекции',
                              widget=forms.TextInput(attrs={
    ...
    class Meta:
        model = Imns
        fields = ['id', 'number', 'name', 'shot_name', 'address',
                  'email', 'post', 'unp']
...
class CICForm(forms.ModelForm):
    ...
    number = forms.CharField(label='№ утвержденного отчета',
                             widget=forms.TextInput(attrs={
                                'placeholder': '№ утвержденного отчета',
                                'class': 'form-control'
                             }))
    date_state = forms.DateField(label='Дата утвержденного отчета',
                                 initial=datetime.date.today,
                                 widget=forms.DateInput(format='%Y-%m-%d', attrs={'type': 'date'}))
    date_from = forms.DateField(label='Изучаемый период с',
                                initial=datetime.date.today,
                                widget=forms.DateInput(format='%Y-%m-%d', attrs={'type': 'date'}))
    date_to = forms.DateField(label='Изучаемый период по', initial=datetime.date.today,
                              widget=forms.DateInput(format='%Y-%m-%d', attrs={'type': 'date'}))
    message = forms.CharField(label='Направлено обзорное письмо',
                              widget=forms.TextInput(attrs={
...
class ExaminationForm(forms.ModelForm):
    id = forms.CharField(initial='', widget=forms.HiddenInput(),
                         required=False)
    ...
    count_contravention = forms.IntegerField(label='Количество нарушений',
                                             widget=forms.TextInput(attrs={
                                              "placeholder": 'Количество документов подвегрнутых контролю',
                                              'class': 'form-control'
                                             }), initial=0)
    description = forms.CharField(label='Краткое содержание нарушения',
                                  widget=forms.Textarea(attrs={
...
class FilterMainForm(forms.Form):
    subject = forms.ModelChoiceField(label='Субъект',
                                     queryset=Imns.objects.order_by('number'),
